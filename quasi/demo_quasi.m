...
puts("Soal 1b)\tLuas permukaan tabung minimum dengan syarat volumenya 400 satuan isi \n");
start = time;
[x, iter] = quasi(@f1b, @g1b, [3.99 7.99 -0.5012]', @(f, x, g, p)(backtrack_armijo(f, 1e-3, x, p, 0.1, 0.5, g)), @dfp, 1e-10);
printf("\t\t<Quasi-Newton: quasi/quasi.m>\n");
printf("\t\tx* = [%15.6f %15.6f %15.6f]^T\n", x(1), x(2), x(3));
printf("\t\texecution time: %.5f s\n", time - start);
printf("\t\t%d iterasi\n", iter);
printf("\t\tB matrix updating method: SR-1 (quasi/sr-1.m)\n");
printf("\t\tLine search method: Backtracking-Armijo (line-search/backtrack_armijo.m)\n\n\n");
